...
instance Applicative (Parser a) where
  pure = return
  (Parser a) <*> b = Parser $ \x -> a x >>= \(f, xs) -> run (fmap f b) xs
  (Parser a) *> (Parser b) = Parser $ \x -> a x >>= \(_, xs) -> b xs
  (Parser a) <* (Parser b) = Parser $ \x -> a x >>= \(f, xs) -> fmap ((,) f . snd) $ b xs
...
rollbackParser (Parser runner) = Parser $ \xs -> runner xs >>= \(result, _) -> Just (result, xs)
